...
#define MEMC(addr)  (*((volatile char*)(addr)))

int uemd_dram_full_test(unsigned long begin, unsigned long end)
{
	debug("Starting mem test from %08lx to %08lx\n", begin, end);
	for(; begin != end; begin++) {
		MEMC(begin) = 0x33;
		if( MEMC(begin) != 0x33) {
			debug("Full memtest failed at %08lx\n", begin);
			return begin;
		}
	}
	return 0;
...
int uemd_em_init_check(struct memregion *reg)
{
	...
	if(em0_size < PHYS_EM0_SIZE) {
		printf("WARNING: SDRAM init returns inappropriate size\n");
		return -1;
	}
