...
static inline struct nand_chip* nand_get_chip(nand_info_t *info)
{
	struct nand_chip* chip = (struct nand_chip*)info->priv;
	if(!chip) return NULL;
	if(chip->magic != NAND_CHIP_MAGIC) return NULL;
	return chip;
}
