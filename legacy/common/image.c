...
extern int do_bdinfo(cmd_tbl_t *cmdtp, int argc, char * const argv[]);
#endif
...
int image_check_dcrc (const image_header_t *hdr, ulong* pdcrc)
{
						...
						int verify)
{
	...
	if (verify) {
		...
		if (!image_check_dcrc (rd_hdr, NULL)) {
			puts ("Bad Data CRC\n");
 ...
 */
int genimg_get_format (const image_header_t * hdr)
{
